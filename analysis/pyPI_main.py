...
import pyPI_constants as constants
import pyPI_utilities as utilities
...
def cape(TP,RP,PP,T,R,P,ascent_flag=0,ptop=50,miss_handle=1):
    ...
    if (np.sum(valid_i) != len(P)):
        ...
        if (miss_handle != 0):
            ...
            return(CAPE_PA,CIN,TOB,LNB,IFLAG)
        else:
...
#         TOB=np.nan
    ...
    #            
        CAPED=PA+PAT-NA
        CAPE_PA=PA+PAT
        CIN=NA # negative area AKA convective inhibition
        CAPED=max([CAPED,0.0])
        CAPE_PA=max([CAPE_PA,0.0])
