...
def get_vars_ifile_special(file, ivar_str, xtime_read, t0, t1, new_p_levels=None):
    ...
    for it_file in range(nt_file):
        ...
        try:
            ...
            var_ifile = var_it.copy(deep=True)
    ds.close()
    return var_ifile, xtime_read

# def get_vars_ifile_2d(file, var_list, xtime_read, t0, t1):
#     ds = Dataset(file)
#     nt_file = ds.dimensions['Time'].size
#     xtime_min = ds.variables['XTIME'][:]
#     split = ds.variables['XTIME'].getncattr('units').split(' ')[2:4]
...
#             except:
#                 vars_ifile[ivar_str] = vars_it[ivar_str].copy(deep=True)
#     ds.close()
#     return vars_ifile, xtime_read
...
def calculate_rainrate(rainnc_all, npd):
    rainrate = rainnc_all.copy(deep=True)
...
def write_ncfile(outdir, var, var_name, tag_extra=''):#, dims_set, pres=None, do_pres=False): #, dim_names
    ...
    try:
        ...
        print('Error in metadata for variable '+var_name)
        pass
    try:
...
# Special variables (including 3D)
def var_list_special():
    return [
